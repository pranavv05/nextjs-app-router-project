...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        const getRandomChar = () => {
          const charset = scrambleCharset || characters;
        ...
        chars.forEach(ch => {
          ...
          const w = ch.getBoundingClientRect().width || 20;
          ...
          for (let k = 0; k < rolls; k++) {
            ...
            if (scrambleCharset) c.textContent = getRandomChar();
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            gsap.set(strips, { x: (i: number, target: HTMLElement) => parseFloat(target.getAttribute('data-start-x') || '0') });
            onShuffleComplete?.();
          },
          onComplete: () => {
            playingRef.current = false;
        ...
        const addTween = (targets: HTMLElement[], at?: number | string) => {
          tl.to(
            targets,
            {
              x: (i: number, target: HTMLElement) => parseFloat(target.getAttribute('data-final-x') || '0'),
              duration,
              ease,
              force3D: true,
  ...
  const validTags = ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'div'] as const;
  const Tag = validTags.includes(tag as any) ? tag : 'p';
